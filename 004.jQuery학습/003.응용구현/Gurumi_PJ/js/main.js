...
$(function(){ //////// jQB (제이쿼리 코드블록) //////////////////
    ...
    *////////////////////////////////////////////////
    let autoSlide = ()=>{  /* 이걸 크게 autoSlide에 담음 // 변수지만 함수다. ()=>{} */
        autoI = setInterval(()=>{ /* 위에서 공용 저장공간에 변수 선언한 "autoI"에 다시 담는다. */
            goSlide(1); // 오른쪽 이동 호출!
        },3000); /// 인터발 함수 /////* ★★★★★★  setInterval(함수,시간) -> 일정시간 간격으로 함수 호출해주는 것 */
    ...
    *////////////////////////////////////////////////////
    let clearAuto = ()=>{
        ...
        // ★★★★★★ 1. 인터발 지우기
        clearInterval(autoI); /* 아까 위에서 autoI에 setInterval 한 내용이 클리어됨 */
        // clearInterval() 인터발함수 지우는 메서드

        // 2. ★★★★★★ 타임아웃 지우기(안지우면 실행 쓰나미가 일어남!!!)
        clearTimeout(autoT); /* 지우는 함수를 호출해서 아래서 지운다. 꼭 여기서 지우고, 아래서 할당을 하면 타임아웃이 하나만 남아서 실행쓰나미가 일어나지 않는다. */
        // 지우고 넣고 하니까 하나만 남는다.
        // 결과적으로 타임아웃 셋팅이 "하나만 남는다"!!!

        // 3. ★★★★★★ 일정시간 뒤 다시 자동호출하기 -> 내가 직접 클릭하고, 4초동안 아무런 움직임 없으면 다시 움직임 : 변수에 할당
        autoT = setTimeout(autoSlide,4000); /* 3초마다 돌아가는게 4초뒤 실행 */
        /* 
