        ...
        */
        ...
        if (spos < -win * 1.1) {
            ...
            /* 
                ...
                기본값인 stop(false,false) -> 시작하다가 마지막애니로 종료!

                ▷ 참고 : FED\004.jQuery학습\001.기본사용법\002.선택자메서드연습\jQuery-02.메서드연습 >> 48.animate_stop.html
            */
            slide.stop().animate({  /* stop() 현재는 처음값으로 돌아가니까 펄쩍펄쩍 뜀  */
                left: -win * 2 + "px"
                ...
                /* 이때 left값을 -100% 위치로 고정해야한다! 
                   왜냐하면 원래 맨앞에 6번슬라이드 이미지가 있었는데, 이걸 맨 뒤로 이동시키니
                   앞이 없지 않은가?
                   원래는 슬라이드 순번이 
                   6 >> 1 >> 2 ... 였는데 이제 6번이 맨 뒤로 가서 맨 앞이 없으니 맨앞이 비었으니
                   지금 이 콜백함수 들어오기 전에는 슬라이드가 2번이 보이니까 left : -200%이었는데
                   앞이 없으니 팅겨보이는것... 
                   그러니 left : -100% 로 조정해주는것!!!
                   한마디로 경계선을 조정해준것!
                */
                .css({left:-win+"px"});
